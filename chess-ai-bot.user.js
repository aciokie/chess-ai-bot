...
(function () {
    ...
    const Platform = {
        ...
        getLichessPlayerColor: (board) => {
            ...
            // 5. Fallback: No API color - use board orientation + visual pieces from chessground
            ...
            // 6. AI GAME FALLBACK: Detect from DOM piece positions (for AI games where chessground not accessible)
            console.log('[SF Engine] Lichess: Trying AI game fallback - detecting from DOM pieces...');
            const domColor = Platform.getLichessColorFromDOMPieces(board);
            if (domColor !== null) {
                console.log('[SF Engine] Lichess: AI game fallback succeeded:', domColor === 1 ? 'WHITE' : 'BLACK');
                return domColor;
            }
            
            // 7. Last resort: visual pieces only from chessground
            if (cg?.state?.pieces) {
                let blackOnRank1 = false;
                for (const [key, piece] of Object.entries(cg.state.pieces)) {
                    if (key.startsWith('1') && piece.color === 'black') {
    ...
    //                     native XHR (respects containers/proxies), patch self.fetch, build Worker from Blob.
    ...
    //
    // format: "wasm"  → JS + WASM fetched via native XHR, self.fetch patched, Worker Blob
